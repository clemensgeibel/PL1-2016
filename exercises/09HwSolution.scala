...
//            eval(le)(r) + eval(re)(r)
  ...
  trait ReaderMonad extends Reader {
  ...
  trait Eval extends Reader {
    ...
    object DesugarForComprehensions {
      def eval(e: Exp): M[Value] = e match {
        case Num(n) =>
          unit(n)
        case Id(x) =>
    ...
    object DesugarForComprehensionsAndTypeInference {
      def eval(e: Exp): M[Value] = e match {
        case Num(n) =>
          unit[Int](n)
        case Id(x) =>
          ask.map[Int](env => env(x))
        case Add(l, r) =>
          //        eval(l).flatMap { lV =>
          //          for {
          //            rV <- eval(r)
    ...
    object InlineMapFlatMapAndTypeInference {
      def eval(e: Exp): M[Value] = e match {
        ...
        case Add(l, r) =>
          bind[Int, Int](eval(l)) { lV =>
            bind[Int, Int](eval(r)) { rV =>
              unit[Int](lV + rV)
            }
          }
      }
  ...
  object Task2 extends Eval with ReaderMonad {
    def eval2(e: Exp): Env => Value = e match {
      case Num(n) => _ => n
      case Id(x) => env => env(x)
      case Add(l, r) =>
        env =>
          eval2(l)(env) + eval2(r)(env)
    }
  }
}
